...
Hooks Architecture avoids boilerplate code and focuses on functionality. The less code lines the better.

### Declarative Programming

In computer science, declarative programming is a programming paradigm that expresses the logic of a computation without describing its control flow.

In react the DOM is declarative. This means we never interact with DOM, the UI is updated when we change the state, it reacts to state changes.
