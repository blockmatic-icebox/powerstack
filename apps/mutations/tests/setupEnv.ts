...
process.env.DATABASE_URL = process.env.DATABASE_URL!.replace('<DB_NAME>', randomDbName);

// Function to create the database
async function createDatabase() {
  // Extract the connection information without the database name from the DATABASE_URL
  const connectionString = process.env.DATABASE_URL!.replace(`/${randomDbName}`, '');
...
async function migrateDatabase() {
  try {
    const { stdout, stderr } = await pexec('yarn migrate:dev', { timeout: 10000 });
