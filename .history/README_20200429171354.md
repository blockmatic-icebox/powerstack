...
export default function useInAppNotification() {
  ...
  if (context === undefined) {
    throw new Error(
      'You must wrap your application with <InAppNotificationProvider /> in order to useInAppNotification().',
    );
  }
...
export function InAppNotificationProvider({ children }: OneOrMoreChildren) {
  const [message, setMessage] = useState<InAppNotificationMessage | null>(null);

  const showNotification = (message: InAppNotificationMessage) => {
    setMessage(message);
    showMessage({ message: '' }); // empty message because we render a custom component instead.
  };

  return (
    <CustomHookContext.Provider value={{ showNotification, message }}>
      {children}
